...
import { EmployeeAuditModel } from "../models/employeeAudit.model.js";
...
export const GetUsersService = async (skip, limit) => {
    const result = await UserModel.findAll({
        where: { is_admin: false, in_bin: false },
        include: [
...
/** Append one audit/history entry. Never throws — auditing must not break the op. */
export const logEmployeeAudit = async ({
  user_id,
  action,
  performed_by = null,
  ...
  try {
    if (!user_id || !action) return null;
    return await EmployeeAuditModel.create({
      user_id,
      action,
      performed_by,
      performed_by_id,
    ...
    console.error("logEmployeeAudit failed:", err?.message || err);
    return null;
  }
};

/** Soft-delete: move an employee to the recycle bin. Returns the user or null. */
export const BinEmployeeService = async (id, actorName = null) => {
  const user = await UserModel.findByPk(id);
  if (!user) return null;
  await user.update({ in_bin: true, binned_at: new Date(), binned_by: actorName });
  return user;
};

/** Restore an employee out of the recycle bin. Returns the user or null. */
export const RestoreEmployeeService = async (id) => {
  const user = await UserModel.findByPk(id);
  if (!user) return null;
  await user.update({ in_bin: false, binned_at: null, binned_by: null });
  return user;
  
};

/**
 * Permanently delete an employee — ONLY if it is currently in the bin
 * (two-step safety). Returns a plain snapshot of the deleted user, or:
 *   null               -> not found
 *   { notInBin: true } -> found but not in the bin (caller should refuse)
 */
export const DeleteEmployeePermanentService = async (id) => {
  const user = await UserModel.findByPk(id);
  if (!user) return null;
  if (!(user.in_bin === true || user.in_bin === 1)) return { notInBin: true };
  const snapshot = {
    _id: user._id,
...
/** List employees currently in the recycle bin. */
export const GetBinnedEmployeesService = async (skip = 0, limit = 50) => {
  const result = await UserModel.findAll({
    where: { is_admin: false, in_bin: true },
    include: [
      { model: RoleModel, as: "userRole" },
      { model: CompanyModel, as: "company" },
      { model: PlantModel, as: "plant" },
    ...
    offset: skip,
    limit,
    order: [["binned_at", "DESC"]],
  });
  return result;
};

/** Full history (audit log) for one employee, newest first. */
export const GetEmployeeHistoryService = async (id) => {
  const result = await EmployeeAuditModel.findAll({
    where: { user_id: id },
