...
export const usePlcDowntimeByError = (filters = {}, options = {}) => {
  const { enabled = true } = options;
  const { startDate, endDate, company_name, plant_name, device_id, model } = filters;

  return useQuery({
    queryKey: ["plc-downtime-by-error", { startDate, endDate, company_name, plant_name, device_id, model }],
    queryFn: async () => {
      const params = {};
      ...
      const res = await axiosHandler.get("/plc-data/analytics/downtime-by-error", { params });
